...
class TokenBucketImpl implements TokenBucket {
	...
	TokenBucketImpl(TokenContainer tokenContainer, RefillStrategy refillStrategy, SleepStrategy sleepStrategy) {
		this.tokenContainer = checkNotNull(tokenContainer);
		checkArgument(tokenContainer.getCapacity() > 0);
		checkArgument(tokenContainer.getSize() <= tokenContainer.getCapacity());
		this.refillStrategy = checkNotNull(refillStrategy);
		this.sleepStrategy = checkNotNull(sleepStrategy);
	}

	public long getSize() {
		return tokenContainer.getSize();
	}

	public void setSize(long size) {
		tokenContainer.setSize(size);
	}
	...
	public long getCapacity() {
		return tokenContainer.getCapacity();
	}
	 ...
	 *
	 * @see org.isomorphism.util.TokenBucket.RefillStrategy#getDurationUntilNextRefill(TimeUnit)
	 * @param unit
	...
	/**
	 * Attempt to add some tokens to the bucket if the refill strategy will
	 * permit it.
	 */
	private void refill() {
		long newTokens = Math.min(getCapacity(), Math.max(0, refillStrategy.refill()));
		setSize(Math.max(0, Math.min(getSize() + newTokens, getCapacity())));
	}
}
